	...
	if($recentposts_mini){
		foreach ($recentposts_mini as $post){
			if( $post->post_type == 'page' ){
				...
				if((!$loc)||(strstr($loc,'jiecao'))){ continue; }
...
function build_baidu_sitemap_html(){
	...
	if($recentposts_html){
		foreach ($recentposts_html as $post){
			if(strstr('<li><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'" target="_blank">'.get_the_title($post->ID).'</a></li>','jiecao')){continue;}
    ...
    if($array_baidu_sitemap_options['lc_category_select']){ 
		$html_category_contents = wp_list_categories('echo=0&exclude=154');
	}
