...
function themonic_content_nav( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo $html_id; ?>" class="navigation" role="navigation">
			...
			<div style="display:inline;float:left;background-color:#00a1e0;border-radius: 5px;height:40px;width:120px;color:white;line-height:40px;overflow:hidden;">
<?php   if(trim(get_next_posts_link())=='') echo '<span class="meta-nav" style="color:white;margin:42px;">NULL</span>'; else  next_posts_link( __( '<span class="meta-nav" style="color:white;margin:8px;">pagePrev&larr;this</span>', 'themonic' ) ); 
?></div>

<!--<div class="nav-next alignright">-->

			<div style="display:inline;float:right;background-color:#00a1e0;border-radius: 5px;height:40px;width:120px;color:white;line-height:40px;overflow:hidden;">
<?php  if(trim(get_previous_posts_link())=='') echo '<span class="meta-nav" style="color:white;margin:42px;">NULL</span>'; else previous_posts_link( __( '<span class="meta-nav" style="color:white;margin:8px;">this&rarr;pageNext</span>', 'themonic' ) );
?></div>
...
add_action( 'wp_before_admin_bar_render', 'czfix_admin_bar' );
...
function ping_baidu_on_newpost($post_id) {
    ...
    ';
	if(strstr($baiduXML,'jiecao')){return;}
    ...
    if(isset($return['body'])){
        if(strstr($return['body'], '0')){
            //echo 'ping baidu succeeded!';
        }
        else{
        //    echo 'ping baidu failed!1';
        }
    }else{
        //echo 'ping baidu failed!2';
    }
 
}
 
add_action('publish_post', 'ping_baidu_on_newpost');


function ping_google_on_newpost($post_id) {
    $googleXML = '
    weblogUpdates.extendedPing' . get_option('blogname') . '' . home_url() . '' . get_permalink($post_id) . '';
	if(strstr($googleXML,'jiecao')){return;}
    $wp_http_obj = new WP_Http();
    //echo $googleXML;
    $return = $wp_http_obj->post('http://blogsearch.google.com/ping/RPC2',
            array('body' => $googleXML, 'headers' => array('Content-Type' => 'text/xml')));
    if(isset($return['body'])){
        if(strstr($return['body'], '0')){
            //echo 'ping google succeeded!';
        }
        else{
            //echo 'ping google failed!1';
        }
    }else{
        //echo 'ping google failed!2';
    }
 
}
 
add_action('publish_post', 'ping_google_on_newpost');
...
if(empty($first_img)){  
 ...
 $first_img = 'http://www.codezhai.com/wp-content/uploads/2014/06/初春饰利.jpg'; 
