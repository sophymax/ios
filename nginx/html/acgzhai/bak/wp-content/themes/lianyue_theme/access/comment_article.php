...
if( $comment_author == 'Name :' )  $comment_author = null;
if( $comment_author_email == 'Email :' )  $comment_author_email = null;
if( $comment_author_url == 'Website :' )  $comment_author_url = null;
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('<strong>ERROR</strong>: please fill the required fields (name, email).') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('<strong>ERROR</strong>: please enter a valid email address.') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('<strong>ERROR</strong>: please type a comment.') ); // 將 wp_die 改為錯誤提示
...
if(!is_user_logged_in()){
	...
	if(config('comment_select') =='开启'){
		...
		if($url){
			...
			$error_url = "/{$error_url}/";
			$error_url = preg_match($error_url,$comment_author_url);
			if($error_url){
				err('<strong>错误</strong>: Website 被禁止'); 
			}
		}
		...
		if($content){
			...
			$error_content = preg_match($error_content,$comment_content);
			if($error_content){
				err('<strong>错误</strong>: Content 被禁止'); 
			}
		}
		...
		if($mail){
			...
			$error_mail = preg_match($error_mail,$comment_author_email);
			if($error_mail){
				err('<strong>错误</strong>: Email 被禁止'); 
			}
		}
	}
}
